...
### 3.3.2 生成完成立刻显示，缓存转后台（v1.6）

老逻辑把下载卡在轮询回调里：

```js
job.status = 'succeeded';              // 此刻已被移出轮询队列
for (...) await fetchAsBlobTrack(...)  // 几十 MB，走隧道
toast('生成完成'); renderJob(job);      // 下载完才肯显示
```

服务器早就出歌了，界面却一直空着、连个提示都没有，**看起来就像卡死**（用户原话："生成歌曲以后好长时间拉不下来"）。现在拆成两步：

- `buildTrack()` **立刻**构造 track 并 `renderJob` + 提示，任务卡马上出现（**注意：可播地址的取法见下面 v1.8 那节，APP 里不能用服务器地址**）；
- `cacheTrack()` 在后台把字节拉下来，成功后换成本地 blob 并入库。

**⚠️ 两个必须记住的约束（v1.7 修的就是这两条）：**

1. **`renderJob` 用 `innerHTML` 重建整张卡片，会把 `new Audio()` 一起扔掉。** 所以缓存完成后**不能无条件整块重绘**——正在播的歌会当场断。最终做法见下面 v1.8 那节：只替换单张卡（`refreshTrackEl`）。
2. **响度增强只对 `blob:` 地址生效**（`createMediaElementSource` 接跨域 http 流会出静音，所以代码里限定了 blob）。于是**刚生成的歌第一次播是没有增强的、偏小声**，等本地缓存落地并重绘后才恢复。这是"立刻能听"的代价；想反过来就得改回等下载完再显示。

#### ❗ v1.6/1.7 的严重回归（v1.8 修复）——别再犯

v1.6 让 track 先用**服务器地址**当 `playUrl`，想着"先能播、缓存后台补"。**在 APP 里这是错的**：页面是 `https://appassets.androidplatform.net` 安全源，服务器是私网 `http://`——**正是 §3.2 里 WebView 拦掉的那一对**，等于把 v1.0 的老 bug 请了回来。现场表现：点播放弹"无法播放音频"，点下载弹"音频还未缓存完成"（`blob` 还是 null）。

**铁律：在 APP 里（`NATIVE === true`），歌只有进了本地缓存才有可播地址。** 所以：

- `buildTrack()` 里 `playUrl: NATIVE ? null : remoteUrl`——浏览器可以直连，APP 不行。
- `playUrl` 为空时 `renderJob` 渲染**占位卡** `buildPendingTrackEl()`（"⏳ 正在下载到本地…"，失败则显示真实原因 + **重试下载**按钮），**而不是一个点了必报错的播放器**。
- 顺带修正：卡片底部那句"…播放/下载会直接访问服务器地址"**在 APP 里是假话**（根本访问不到），已限定为仅浏览器显示。当年排查被它误导过一次，见 3.3.1。

**还有一个连带缺陷**：批量出 2 首时，若按"有歌在播就不重绘"的做法，A 在播会让 B 永远卡在"下载中"。所以缓存完成后**只替换那一张卡**（`refreshTrackEl(job, t, idx)`，卡片 id = `track-<jobId>-<idx>`）：占位卡身上没有播放器（`data-pending="1"`），换它永远不会断歌；只有替换**已带播放器**的卡时才需要避让正在播放的情况。
...
| 1.8 | ❗修 1.6/1.7 的严重回归：APP 内不能拿服务器地址当可播地址（3.3.2）；未缓存好改用占位卡+重试；缓存完成只换单张卡 |
