...
- `buildTrack()` 用**服务器地址**直接构造 track，**立刻 `renderJob` + 提示**，点开就能播；
- `cacheTrack()` 在后台把字节拉下来，成功后换成本地 blob 并入库；**失败也不致命**，继续播服务器地址（和 3.3.1 的兜底一致）。

**⚠️ 两个必须记住的约束（v1.7 修的就是这两条）：**

1. **`renderJob` 用 `innerHTML` 重建整张卡片，会把 `new Audio()` 一起扔掉。** 所以缓存完成后**不能无条件重绘**——正在播的歌会当场断。用 `renderJobUnlessPlaying()`：`allAudios` 里只要有一个在播就跳过，等安静了再换（数据早就写在 track 上，晚换不丢东西）。
